...
struct UniLinkedList {
    ...
    UniLinkedList(int* p, size_t n) : size_(n) {
        ...
        for (size_t i = 0; i < n; ++i) {
            cur->reset(new Node(p[i]));
            cur = &((*cur)->next);
        }
    }
    UniLinkedList(const UniLinkedList& other) : size_(other.size_){
        auto cur = &root_;
        for (auto curOther = other.root_; curOther != nullptr; curOther = curOther->next) {
            cur->reset(new Node(curOther->val));
            cur = &((*cur)->next);
        }
    }
    ...
    UniLinkedList(size_t n, int val_default=0) {
        auto cur = &root_;
        while (n--) {
            cur->reset(new Node(val_default));
            cur = &((*cur)->next);
        }
    }
